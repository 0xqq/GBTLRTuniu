          ...
          attr match {
            ...
            case nomAttr: NominalAttribute =>
              nomAttr.getNumValues match {
                ...
                case None => throw new IllegalArgumentException(s"javaUtils.Feature $idx is " +
...
class GBTLRClassificationModel (
  ...
  /**
    ...
    *         获取每个样本的组合特征(rowFeatures, gbdtFeatures)
    */
  def getComibinedFeatures(
    ...
    // 传入每棵树的根节点，寻找该树的叶子节点. 注意getLeafNodes是一个递归函数
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    val datasetWithCombinedFeatures = dataset.withColumn($(gbtGeneratedFeaturesCol),
      addFeatureUDF(col($(featuresCol))))
    // transform 产生三列：predictionCol, rawPredictionCol, probabilityCol
